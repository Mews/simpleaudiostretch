...
class TestAudioFunctions(unittest.TestCase):
    ...
    def setUp(self):
        ...
        duration = 2  # seconds
        t = np.linspace(0, duration, int(self.sine_samplerate * duration), endpoint=False)
        self.sine_audio = 0.5 * np.sin(2 * np.pi * 440 * t)  # Generate a 440 Hz sine wave
    

    def test_stretch_audio_raw(self):
        factor = 2

        stretched_audio, stretched_samplerate = stretch_audio(audio=self.sine_audio, factor=factor, samplerate=self.sine_samplerate)
    ...
    def test_speedup_audio_raw(self):
        ...
        spedup_audio, spedup_samplerate = speedup_audio(audio=self.sine_audio, factor=factor, samplerate=self.sine_samplerate)
    ...
    def test_stretch_save_audio_file(self):
        ...
        stretch_audio(audio=self.sine_audio, factor=factor, output=out_file, samplerate=self.sine_samplerate)
    ...
    def test_speedup_save_audio_file(self):
        ...
        speedup_audio(audio=self.sine_audio, factor=factor, output=out_file, samplerate=self.sine_samplerate)
